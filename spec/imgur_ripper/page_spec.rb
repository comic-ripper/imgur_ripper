...
describe ImgurRipper::Page, vcr: true do
  subject(:page) { ImgurRipper::Page.new url: url, number: number }

  let(:url) do
    "http://www.bato.to/read/_/88615/100-is-too-cheap_by_peebs/1"
  end
  ...
  describe "JSON Serialization / Unserialization" do
    ...
    context "The image_url has been retrieved" do
      subject(:page) do
        ImgurRipper::Page.new url: url, number: number, image_url: image_url
      end
