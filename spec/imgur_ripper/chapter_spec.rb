...
describe ImgurRipper::Chapter, vcr: true do
  subject(:chapter) { ImgurRipper::Chapter.new url: url, text: link_text }
  let(:link_text) { "Ch.0: [Oneshot]" }
  let(:url) { "http://www.bato.to/read/_/88615/100-is-too-cheap_by_peebs" }

  describe "#pages" do
    it "creates a Page" do
      expect(chapter.pages.first).to be_a ImgurRipper::Page
    end
